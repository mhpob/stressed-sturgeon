...
dat <- data.table::fread("sturgeon_detections.gz")
...
locs2020<- read.csv("2020_receivers.csv")
...
###----------------------------------------###
### load VPS detections
VPSdat <- read.csv("2020_VPS_Detections.csv")
...
VPSdat$DATETIME<- mdy_hm(VPSdat$DATETIME,tz=Sys.timezone())
...
plot(locs2020[,c("long","lat")], col="red", cex=1, main="A69-9001-23904")
points(dat2020[dat2020$transmitter=="A69-9001-23904", c("long","lat")], col="blue", pch=16, 
       cex=table(dat2020[dat2020$transmitter=="A69-9001-23904", c("station")])/max(table(dat2020[dat2020$transmitter=="A69-9001-23904", c("station")]))*2)
...
for(i in 1:ntelem){
  ...
  for(k in 1:K){
    if(length(as.numeric(names(table(tmp [which(as.Date(tmp[,"date.local"])==days[k]),"reach"]))))>0){
      y[i,k] <- ifelse(3 %in% as.numeric(names(table(tmp [which(as.Date(tmp[,"date.local"])==days[k]),"reach"]))), 3, 
                       max(as.numeric(names(table(tmp [which(as.Date(tmp[,"date.local"])==days[k]),"reach"])))))
    }#if 
  }#k
  ...
  if(ID2020[i] %in% VPSid){
    ...
    for(k in 1:K){
      if(length(as.numeric(names(table(tmpVPS[which(as.Date(tmpVPS[,"DATETIME"])==days[k]),"reach"]))))>0){
...
SturgeonCapDat <- read.csv("sturgeon_capture_data.csv")
firstTelem <- as.Date(x = integer(0), origin = "1970-01-01")
startTelem<-lastTelem<-NA
for(i in 1:(ntelem-1)){ ## 21909 has a different history (tagged in another system)
  firstTelem[i] <- as.Date(SturgeonCapDat$ DateCaptured[which(SturgeonCapDat$TransmitterNumber==ID2020[i])], format="%m/%d/%Y")
  startTelem[i] <- ifelse(firstTelem[i]<StartDate, 1, as.numeric(difftime(firstTelem[i], as.Date(StartDate), units="days"))+1)
...
###----------------------------------------###
### Load 2021 SSS data
sssDat <- read.csv("2020_Total_SSS2.csv")
...
sssDat$SurveyID <- as.numeric(sssDates[,2][match(sssDat$Date.Collected, sssDates[,1])])

sssMat <- table(sssDat$SurveyID , sssDat$Pass)
sssSurveyOcc <- as.numeric(unique(sssDat$Date.Collected)-as.Date(StartDate)+1) # occasion of survey relative to StartDate
...
###----------------------------------------###
### load temp data
tempDat <- read.csv("2020_Temp.csv")
Temp <- ggplot() + geom_point(data=tempDat, aes(x=Date, y=Temp)) + 
  ggtitle("2021") +
  xlab("") + 
  ylab("Temperature (\u00B0C)") + 
  theme_classic() + 
  theme(axis.text = element_text(size = 12,), axis.title.x = element_text(size = 14), plot.title = element_text(hjust=1)) 
Temp
...
jags.data <- list(
  ...
  a=rep(1,5),
  J=J, 
  temp=tempDat$Seaford[11:83] #Change this for 2021 temp data
)# data


params <- c("LambdaSuper","N","NsuperK","Nreach","R","Nsuper","Nmc", "Nnan",
            "p_sss",
            "pbar", 
            "psi",
            "theta",
            ...
            "phi","phi0", "alpha1",
            "b","eta",
            "Mnanticoke","Mlnr","Mlmc","Mumc","Mmc",
            "M","Msuper","mR",
            "expectedProbabilities", "moveProb2", "moveProb5","entryProb")
...
for(i in 1:ntelem){
  ...
  if(lastTelem[i]<K) { zInits[i,(lastTelem[i]+1):K] <- 6 }# just have them leave the system to start
  if(1%in%zInits[i,startTelem[i]:lastTelem[i]]){zInits[i,startTelem[i]:lastTelem[i]][which(zInits[i,startTelem[i]:lastTelem[i]]==1)]<-2}
}
...
NreachInits[1,1:5] <- as.vector(rmultinom(1, NsuperKinit, c(.55,.15,.1,.05,0.15)))
...
nc <- 5; nAdapt=25000; nb <- 10000; ni <- 50000+nb; nt <- 10;

# Call JAGS from R 
out <- jags(jags.data, inits, params, "Sturgeon_SSS_Telem_MS_ExpectedProb_test.txt", 
...
plot(out$q50$N, ylim=c(0,50), xaxt="n", xlab="Date", ylab="Sturgeon (marked+unmarked)", las=1, pch=16)          # total marked in system by day
...
legend("topright", legend=c("Nanticoke system", "Nanticoke reaches", "MC reaches"), text.col=c("black", "red", "blue"), bty='n', cex=1.2)

#Plot UNR daily abundance vs MC abundance 
plot(out$q50$Nmc, ylim=c(0,25), xaxt="n", xlab="Date", ylab="Sturgeon (marked+unmarked)", las=1, pch=16)
...
legend("topright", legend=c("Marshyhope Creek Reaches", "Upper Nanticoke Reach"), text.col=c("black", "blue"), bty='n', cex=1.1)

# plot daily abundance and cumulative abundance of marked individuals
plot(out$q50$Nsuper, ylim=c(0,60), pch=16, , xaxt="n", xlab="Date", ylab="Sturgeon (marked+unmarked)", las=1)          # total marked in system by day
...
legend("topleft", legend=c("Cumulative", "Daily"), text.col=c("black", "red"), bty='n', cex=1.2)
...
plot(out$q50$phi, ylim=c(0.80,1.00), xaxt="n", xlab="Date", ylab="Daily persistence probability", las=1, pch=16)          # total marked in system by day
segments(1:K,out$q2.5$phi, 1:K, out$q97.5$phi) 
axis(1, at=1:K, format(days, '%m-%d'))

plot(jags.data$temp, out$q50$phi, ylim=c(0.80,1.00), xlab="Temp", ylab="Daily persistence probability", las=1, pch=16)          # total marked in system by day
segments(jags.data$temp,out$q2.5$phi, jags.data$temp, out$q97.5$phi) 

#Temperature overlaying Persistence
plot(out$q50$phi, xaxt="n", ylim=c(0.80,1.00), xlab="Date", ylab="Daily persistence probability", las=1, pch=16)
segments(1:K,out$q2.5$phi, 1:K, out$q97.5$phi) 
axis(1, at=1:K, format(days, '%m-%d'))
par(new=TRUE)
#par(mar=c(5.3, 4.3, 4.3, 4.3))
plot(jags.data$temp, pch=16, col="red", axes=FALSE , xlab = "", ylab = "", ylim = c(12,30))
...
axis(1, at=1:K, format(days, '%m-%d'))
par(new=TRUE)
plot(flow$average, pch=16, col="blue", axes=FALSE , xlab = "", ylab = "", ylim = c(0,400))
axis(side=4, xlab="", ylab="")
mtext("Flow (ft^3)/s)", side= 4, line = 3)


#Idea: plot recruitment probs. 
plot(out$q50$b, ylim=c(0.00,0.26), xaxt="n", xlab="Date", ylab="Daily recruitment probability", las=1, pch=16)          # total marked in system by day
