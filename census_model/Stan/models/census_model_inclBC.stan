...
parameters {
  ...
  simplex[5] init_probs;   // Beginning dist (LNR, LMC, UMC, UNR, BC)       
  simplex[5] entry_probs;  // Dist of new entrants (LNR, LMC, UMC, UNR, BC)
  ...
  simplex[5] move_probs_BC;   // From BC to...
...
transformed parameters {
  ...
  // Rows: From, Cols: To. States: 1:NE, 2:LNR, 3:LMC, 4:UMC, 5:UNR, 6:BC, 7:Exit
  for(t in 1:(K-1)){
    ...
    Gamma[t, 1, 6] = eta_t * entry_probs[5]; // to BC
    Gamma[t, 1, 7] = 0;
    ...
    Gamma[t, 2, 6] = phi[t] * fmax(move_probs_LNR[5], 1e-9); // to BC
    ...
    // FROM BC (6)
    Gamma[t, 6, 1] = 0;
    Gamma[t, 6, 2] = phi[t] * fmax(move_probs_BC[1], 1e-9);
    Gamma[t, 6, 3] = phi[t] * fmax(move_probs_BC[2], 1e-9); 
    Gamma[t, 6, 4] = phi[t] * fmax(move_probs_BC[3], 1e-9);
    Gamma[t, 6, 5] = phi[t] * fmax(move_probs_BC[4], 1e-9);
    Gamma[t, 6, 6] = phi[t] * fmax(move_probs_BC[5], 1e-9);
    Gamma[t, 6, 7] = 1 - phi[t];
  ...
  // Obs: 1=NotSeen, 2=LNR, 3=LMC, 4=UMC, 5=UNR, 6=BC
  for(i in 1:G){
    for(k in 1:K){
...
model {
  ...
  move_probs_LNR ~ dirichlet(rep_vector(1, 5)); // pi row 2 (from LNR)
  move_probs_LMC ~ dirichlet(rep_vector(1, 5)); // pi row 3 (from LMC)
  move_probs_UMC ~ dirichlet( [10, 10, 10, 2, 2]' ); // pi row 4 (from UMC, less likely to go to UNR/BC)
  move_probs_UNR ~ dirichlet( [10, 10, 2, 10, 10]' ); // pi row 5 (from UNR, less likely to go to UMC)
  move_probs_BC ~ dirichlet( [5, 5, 2, 10, 15]' ); // pi row 6 (from BC, less likely to go to UMC)
...
generated quantities {
  ...
  vector[K] N_BC = LambdaSuper * col(state_probs, 6);
  vector[K] N = N_LNR + N_LMC + N_UMC + N_UNR + N_BC;
  ...
  vector[K] M_BC = Msuper * col(state_probs, 6);
  vector[K] M = M_LNR + M_LMC + M_UMC + M_UNR + M_BC;
}
