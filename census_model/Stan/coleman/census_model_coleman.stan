functions {
  ...
  real forward_alg(
    ...
    vector[n_states] incoming_paths;
    ...
    // --- DAY 1 ---
    // Use fmax to stop probing probabilities less than 1e-20
    for (s in 1:n_states) {
      // Log of the probability of starting in state s
      log_init = log(fmax(start_dist[s], 1e-20));
      // Log of probability of seeing what we saw on day 1 given the state
      log_seen = log(fmax(rho[1][s, y[1]], 1e-20));

      alpha[1][s] = log_init + log_seen;
    }

    // --- DAY 2 to K ---
    for (t in 2:K) {
      // Transitions (Gamma in Stan parlance; Omega in Coleman et al. 2024)
      for (s_current in 1:n_states) {
        for (s_prev in 1:n_states) {
          // Log of the probability at time t-1 of moving from the previous state
          //  to the current state
          log_trans = log(fmax(gamma_mat[t-1][s_prev, s_current], 1e-20));
          // Fill out path history
          incoming_paths[s_prev] = alpha[t-1][s_prev] + log_trans;
        }
        // Probability of s_current is sum of all incoming paths
        gamma_next[s_current] = log_sum_exp(incoming_paths);
      }

      // Update with Observation for Day t
      for (s in 1:n_states) {
        log_seen = log(fmax(rho[t][s, y[t]], 1e-20));
        alpha[t][s] = gamma_next[s] + log_seen;
      }
    }
