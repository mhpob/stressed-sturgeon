library(cmdstanr)
# compile model (once)
# m <- cmdstan_model(stan_file = "census_model_inclUUNR.stan")

# load model
m <- cmdstan_model(
  stan_file = "census_model_inclUUNR.stan",
  exe_file = "census_model_inclUUNR"
)
...
init_fn <- function() {
  ...
  list(
    ...
    move_probs_LMC = add_jitter(c(0.22, 0.45, 0.22, 0.08, 0.03)), # Harder to jump to the UNR/UUNR fork
    move_probs_UMC = add_jitter(c(0.12, 0.30, 0.50, 0.06, 0.02)), # UUNR is highly improbable from here
    ...
    # UUNR Specific behavior initialization (Stay > UNR > LNR=LMC > UMC)
    # Proportions: Stay (0.41), UNR (0.27), LNR (0.13), LMC (0.13), UMC (0.06)
    move_probs_UUNR = add_jitter(c(0.13, 0.13, 0.06, 0.27, 0.41)),
...
fit$save_object(file = "out2024_UUNR.RDS")
