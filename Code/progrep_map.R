...
#### Seaford ----
library(sf)
seaford <- st_read('data/seaford.json',
                   query = "SELECT * FROM seaford WHERE NAME = 'Seaford'") 
...
ggplot() +
  geom_sf(data = seaford, fill = 'yellow') +
  geom_sf(data = nan) +
  coord_sf(label_axes = '--EN',
           xlim = c(-75.65, -75.57), ylim = c(38.629, 38.687))


library(dplyr)
vps <- st_read('s:/Stressed Sturgeon_2022-2024/VPS_Camera_deploym_08-AUG-23.gpx',
               layer = 'waypoints') |> 
  filter(!grepl('^Marsh', name))
vps[1, 'name'] <- 'Upriver Camera'
vps[2, 'name'] <- 'Downriver Camera'


ggplot() +
  geom_sf(data = seaford, fill = 'yellow') +
  geom_sf(data = nan) +
  geom_sf(data = vps, aes(color = grepl('Camera', name))) +
  coord_sf(label_axes = '--EN',
           xlim = c(-75.63, -75.61), ylim = c(38.629, 38.64))
